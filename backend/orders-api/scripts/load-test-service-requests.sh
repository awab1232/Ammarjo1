...
# Service requests + ratings stress simulation (sequential curl only).
...
probe_get "/service-requests?limit=20" "GET /service-requests"
if [[ "${MOCK_MODE}" == "1" ]]; then
  probe_post "/service-requests" "POST /service-requests (mock invalid safe)" '{"conversationId":"","description":"mock load test","dryRun":true}'
  probe_post "/ratings" "POST /ratings (mock invalid safe)" '{"targetType":"technician","targetId":"mock","rating":0,"dryRun":true}'
else
  probe_post "/service-requests" "POST /service-requests" '{"conversationId":"load-test-conv","description":"Load test request"}'
fi
