...
import 'firebase_uid_resolver.dart';
...
/// ?????? ?? `senderId` / `receiverId` / `timestamp` (ServerTimestamp).
class UnifiedChatRepository {
  ...
  /// Resolve peer UID from backend resolver.
  Future<String?> _lookupPeerFirebaseUid(String peerEmail) async {
    final key = _normEmail(peerEmail);
    if (key.isEmpty) return '';
    try {
      final uid = await FirebaseUidResolver.resolveUidByEmail(key);
      if (uid.isNotEmpty) return uid;
    } on Object {
      debugPrint(
        'UnifiedChatRepository._lookupPeerFirebaseUid resolve failed',
      );
    }
    return '';
  }
  ...
  Future<String> ensureChat({
    ...
    if (me.isEmpty || peer.isEmpty) throw ArgumentError('???? ??????? ?????');
    if (me == peer) throw ArgumentError('?? ???? ??????? ?? ????');
    ...
    if (sellerUid.isEmpty) {
      // `fetchSignInMethodsForEmail` ????? ?? firebase_auth 6+ (????? ?? ???? ??????).
      throw StateError(
        '???? ??? ???? ????? ????? ?????? ????????. ??? ?? ????? ????? ????? ?? ??????? ???? '
        '?? ??? ?????? UID ?? ???? ??????. '
        '(??? UID ??????? ??? ??? ???? ?? ?????? - ?? ????? Cloud Function.)',
    ...
    await _db.runTransaction((tx) async {
      ...
      if (!snap.exists) {
        tx.set(ref, {
          ...
          'lastMessagePreview': seedProductCard ? '?? ????? ????' : '',
        });
  ...
  Future<void> sendText({
    ...
    if (user == null) throw StateError('??? ????? ?? Firebase');
    debugPrint('CHAT INIT');
  ...
  Future<void> ensureParticipantUidOnChat(
    ...
    try {
      ...
      if (!snap.exists) {
        debugPrint('ensureParticipantUidOnChat: ?? ????? ?????? ?? $chatId');
        return;
      }
      final d = snap.data() ?? {};
      final me = _normEmail(currentUserEmail);
