...
/// Non-blocking backend probes — **never** throws; logs only (app must boot offline-degraded).
  ...
  static Future<void> probeBackendHealthNonBlocking() async {
    ...
    try {
      ...
      if (health.statusCode < 200 || health.statusCode >= 500) {
        debugPrint(
          '[RuntimeSafetyEnforcer] /health non-OK ${health.statusCode} — continuing boot',
        );
      } else {
      ...
      if (internalKey.trim().isNotEmpty) {
        try {
          ...
          if (detailed.statusCode < 200 || detailed.statusCode >= 300) {
            debugPrint(
              '[RuntimeSafetyEnforcer] /internal/health/detailed ${detailed.statusCode} — ignored',
            );
          }
        } on Object catch (e, st) {
          debugPrint('[RuntimeSafetyEnforcer] internal health probe failed (ignored): $e\n$st');
        }
      }
    } on Object catch (e, st) {
      debugPrint('[RuntimeSafetyEnforcer] health probe failed (ignored): $e\n$st');
    }
  }
}
